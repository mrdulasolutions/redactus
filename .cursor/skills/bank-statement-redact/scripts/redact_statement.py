...
def cmd_redact(args: argparse.Namespace) -> None:
    payload = extract_payload(Path(args.pdf))
    extract_path = Path(args.workdir) / "extract.json"
    plan_path = Path(args.workdir) / "plan.json"
    ...
    dump_json(result, Path(args.workdir) / "verify.json")
    print(f"Wrote {args.output}")
    print(f"matches={plan['match_count']} regions={len(plan['regions'])} verify_ok={result['ok']}")
    if not result["ok"]:
        raise SystemExit(3)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Keep-search redaction for bank-statement PDFs")
    ...
    s = sub.add_parser("redact", help="extract + plan + apply + verify in one shot (after user confirmed)")
    s.add_argument("pdf")
